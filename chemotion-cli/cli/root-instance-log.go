		...
		if isInteractive(false) {
			...
			if ownCall(cmd) {
				...
				if cmd.Flag("all").Changed && toBool(cmd.Flag("all").Value.String()) {
					...
					if cmd.Flag("service").Changed {
						...
						if elementInSlice(logOf[0], &services) == -1 {
							zboth.Fatal().Err(fmt.Errorf("service not found")).Msgf("No service called %s found associated with the instance called %s.", logOf[0], currentInstance)
						}
					} else {
						logOf = []string{selectOpt(services, "Please select the service whose logs you want")}
					}
				}
				args := fmt.Sprintf("--tail %s", cmd.Flag("tail").Value.String())
				...
				if cmd.Flag("since").Changed {
					args += fmt.Sprintf(" --since %s", cmd.Flag("since").Value.String())
				}
				if cmd.Flag("until").Changed {
					args += fmt.Sprintf(" --until %s", cmd.Flag("until").Value.String())
				}
			...
			zboth.Fatal().Err(fmt.Errorf("illegal operation")).Msgf("Logs can't be printed in quiet mode.")
		}
	},
}
...
func init() {
	...
	logInstanceRootCmd.Flags().Bool("details", false, fmt.Sprintf("--details flag as received by %s logs command", virtualizer))
	logInstanceRootCmd.Flags().Bool("follow", false, fmt.Sprintf("--follow flag as received by %s logs command", virtualizer))
	logInstanceRootCmd.Flags().BoolP("timestamps", "t", false, fmt.Sprintf("--timestamps flag as received by %s logs command", virtualizer))
	logInstanceRootCmd.Flags().String("since", "", fmt.Sprintf("--since flag as received by %s logs command", virtualizer))
	logInstanceRootCmd.Flags().String("until", "", fmt.Sprintf("--until flag as received by %s logs command", virtualizer))
	logInstanceRootCmd.Flags().StringP("tail", "n", "all", fmt.Sprintf("--tail flag as received by %s logs command", virtualizer))
}
