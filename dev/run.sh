...
if [[ "${CONFIG_ROLE}" == "eln" ]]; then
    ...
    (cd /embed/ && make all-dev 2>&1 | grep --line-buffered -v -e "warning: already initialized constant" -e "warning: previous definition of" -e "warning: URI.escape is obsolete")
    [[ -n ${DROP} ]] && export HOME=/chemotion/app
    mkdir -p log/
